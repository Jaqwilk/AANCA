...
<header class="site-header" id="site-header">
  <div class="nav-shell">
    ...
    <button class="menu-button" id="menu-button" type="button" aria-expanded="false" aria-controls="nav-links">Menu</button>
    <nav class="nav-links" id="nav-links" aria-label="Presentation sections"><a href="#overview">Study</a><a href="#method">Method</a><a href="#results">Findings</a><a href="#evidence">Evidence</a><a href="#use">Reproduce</a><a href="#author">Author</a></nav>
  </div>
</header>
...
<main id="main">
  ...
  <section class="section" id="interpretation">
    ...
    <div class="chapter-copy reveal"><p>The controlled positive event is an injected label change. Performance against it measures retrieval of that injected process, not naturally occurring annotation inconsistency, expert disagreement or biological truth. The bootstrap quantifies uncertainty within this dataset; it is not an independent replication.</p><p>Splits separate source patches, not verified patients or whole slides. Their controlled allocation also used the pre-corruption reference label for balance. That label was not a model input, but it is benchmark-only information unavailable in a real audit.</p><p>External validity still requires blinded multi-rater review, patient- or WSI-level grouping where identifiers permit it, a genuinely external dataset and a prospectively defined evaluation of the ranked queue.</p></div>
  ...
  <section class="section" id="use">
    <div class="repo-shell">
      <div class="reproduction-intro reveal"><h2>Read the evidence first; run the software when a deeper check is needed.</h2><p>The checked-in article opens without a dataset, model run or GPU. <code>present_demo.py --verify-only</code> verifies the five-file presentation package; it does not retrain a model or recalculate any primary result. The separate synthetic path exercises the portable software workflow.</p><p>Independent primary recomputation requires a lawful PanNuke copy and full prediction and statistical artifacts that are not currently distributed in the public repository. The repository never downloads the dataset silently or relaxes the scientific gates.</p></div>
      <article class="repo-card reveal">
        ...
        <p class="repo-description">The repository contains the scientific specification, source code, cross-platform software checks, synthetic workflows and this checksum-verifiable presentation. Raw PanNuke binaries and the full primary OOF, bootstrap, embedding and checkpoint artifacts are not distributed, so the public checkout does not independently recompute H1-H7.</p>
...
<footer id="footer"><div class="footer-inner"><div class="footer-divider"></div>
  <div class="footer-grid">
    ...
    <div class="footer-column"><h3>Inspect</h3><a href="evidence.json">Machine-readable evidence</a><a href="README.md">Presentation package notes</a><a href="#evidence">Reproducibility boundary</a><a href="https://github.com/Jaqwilk/AANCA" target="_blank" rel="noopener">GitHub repository ↗</a><a href="https://github.com/Jaqwilk/AANCA/blob/main/ETHICS_AND_LIMITATIONS.md" target="_blank" rel="noopener">Ethics and limitations ↗</a><a href="https://github.com/Jaqwilk/AANCA/blob/main/references/references.bib" target="_blank" rel="noopener">Bibliography ↗</a></div>
  </div>
...
def _render_hypothesis_ledger_v4(
    ...
    entries = (
        ...
        (
            ...
            f"{_format_metric_v3(h4['point_difference'])} macro-F1, so better retrieval "
